...
import * as S from '../../outils/sha-outils';
...
export class NewBlocComponent implements OnInit, OnDestroy {
	...
	newBloc.hashPrecedent = this.blocPrecedent.hashCourant;

	let stringToBeHashed = (newBloc.index + newBloc.hashPrecedent+ newBloc.horodatage+ newBloc.contenu).toString();
	console.log('Dans', here,'stringToBeHashed',stringToBeHashed);
	newBloc.hashCourant = S.createSha(stringToBeHashed,'SHA-512', 'TEXT', 'HEX');
	console.log('Dans',here,'newBloc', newBloc);
